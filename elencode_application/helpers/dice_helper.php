...
function dice_single_roll($dice_max,$mod){
  $roll_value=rand(1,$dice_max);
  return ($roll_value+$mod);
}

function dice_multi_roll($dices){
	if(!is_array($dices)) return 0;
  $roll_value=0;
  foreach($dices as $single_dice){
		$roll_value+=rand(1,$single_dice);
  }
	return $roll_value;
}
 ?>