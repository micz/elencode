...
class Abilities {
  ...
  {
    if($this->Names=$this->CI->elencache->load('abilities.php')){
      return true;
    }else{
      $this->Names=array();
    }

    if(!$query=$this->CI->db->query('SELECT id,name FROM el_abilities ORDER BY id ASC'))
      return false;
    ...
    $this->CI->elencache->save('abilities.php',$this->Names);
